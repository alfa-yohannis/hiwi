defmodule HiwiWeb.Queue.QueueController do
    ...
    # 1. UPDATE PLUG: Tambahkan :show_view_page
    plug(
      HiwiWeb.Plugs.RequireAuth
      when action in [
        :show_registration_page, :show_edit_page, :register, :edit, :delete,
        :assign_teller, :remove_teller, :reset_number, :increment_number,
        :show_view_page # <--- TAMBAHAN UNTUK MONITOR
      ]
    )

    plug(:check_queue_owner when action in [
      :edit, :delete, :assign_teller, :remove_teller, :reset_number, :increment_number
      # Note: show_view_page tidak dimasukkan ke sini agar Teller juga bisa melihat monitor
    ])
    ...
    # --- RESET ---
    def reset_number(conn, %{"id" => id}) do
      queue = Queues.get_queue!(id)

      case Queues.reset_queue_number(queue) do
        {:ok, _queue} ->
          conn
          |> put_flash(:info, "Nomor antrean berhasil di-reset ke 0.")
          |> redirect(to: ~p"/queues")

        {:error, _changeset} ->
          conn
          |> put_flash(:error, "Gagal mereset antrean.")
          |> redirect(to: ~p"/queues")
      end
    end

    # --- INCREMENT ---
    def increment_number(conn, %{"id" => id}) do
      queue = Queues.get_queue!(id)

      case Queues.increment_queue_number(queue) do
        {:ok, _queue} ->
          conn
          |> put_flash(:info, "Antrean maju! Nomor diperbarui.")
          |> redirect(to: ~p"/queues")

        {:error, _} ->
          conn
          |> put_flash(:error, "Gagal menaikkan nomor antrean.")
          |> redirect(to: ~p"/queues")
      end
    end

    # --- VIEW MONITOR (Fungsi Baru) ---
    def show_view_page(conn, %{"id" => id}) do
      queue = Queues.get_queue!(id)
      # Render file show_view.html.heex
      render(conn, :show_view, queue: queue)
    end
    # ----------------------------------
