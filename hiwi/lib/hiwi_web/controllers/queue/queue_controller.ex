defmodule HiwiWeb.Queue.QueueController do
    ...
    # 1. Update Plug: Tambahkan :reset_number agar butuh login
    plug(
      ...
      when action in [
        ...
        :assign_teller,
        :remove_teller,
        :reset_number # <-- TAMBAHAN BARU
      ]
    )

    # 2. Update Plug: Tambahkan :reset_number agar HANYA OWNER yang bisa akses
    plug(:check_queue_owner when action in [:edit, :delete, :assign_teller, :remove_teller, :reset_number]) # <-- TAMBAHAN BARU
    ...
    @doc """
    Plug untuk memastikan hanya owner antrian yang bisa mengakses action tertentu.
    """
      ...
      case Queues.get_queue!(queue_id) do
        %Hiwi.Queues.Queue{owner_id: owner_id} = _queue ->
          if owner_id == conn.assigns.current_user.id do
            ...
            |> redirect(to: "/queues") # Sedikit perbaikan redirect agar lebih aman
            |> halt()
          end
